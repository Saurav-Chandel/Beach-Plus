...
class SignupAPI(generics.GenericAPIView):
    ...
    def post(self, request, *args, **kwargs):#,exception):
        ...
        # url='http://localhost:8000/BeachPlus/api/token/'
    ...
    def post(self,request,*args,**kwargs):
        ...
        if len(request.FILES.getlist('BusinessImages'))>0:
            ...
            for i in range(len(files)):
                fs = FileSystemStorage(location='/home/aashima/BeachPlus/BeachPlus/static_media/BusinessImages/')
                ppn=str(datetime.now())+'.jpg'
                filename = fs.save(ppn, files[i])
                uploaded_file_url = fs.url(filename)
                Pstring.append(uploaded_file_url)
            
            # RA.BusinessImages=Pstring
            #RA.save()
...
class BusinessRecordSerializer(generics.GenericAPIView):
    ...
    def post(self,request,*args,**kwargs):
        OnlineUsers.objects.filter(Q(DateOfRecord__date__lt=datetime.now().date()) | Q(CloseTime__lt=str(datetime.now().time()))).delete()    #deletes the previous dated checkin objects.
...
class ForgotPasswordAPI(generics.GenericAPIView):
    def post(self, request, *args, **kwargs):
        ...
        if len(u)==0:
            ...
            message = Mail(from_email='beachplusbr@gmail.com',to_emails=request.data['email'],subject="Reset Password BeachPlus",html_content=message)
    ...
    def post(self,request,*args,**kwargs):
        ...
        if ser.is_valid():
            ...
            if Round=='3':
                ...
                both_team_players=list(t1_players)+list(t2_players)
                Profiles.objects.filter(User__in=both_team_players).update(MatchesPlayed=F('MatchesPlayed')+1)    
